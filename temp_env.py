...
class DevSecOpsEnvironment(Environment):
    ...
    def step(self, action: DevSecOpsAction) -> DevSecOpsObservation: # type: ignore[override]
        ...
        if action.action_type == "update_package":
            ...
            # 0.99 is the "perfect" score under the strictly-in-(0,1) grader
            if grade_score >= 0.99:
                reward += 0.5
            done = True
            obs.reward = max(min(obs.reward + reward, 1.0), -1.0)
    ...
    def grade(self) -> float:
        ...
        Score bands:
            0.01  anti-cheat violation (required package deleted)
            0.05  build failed
            0.50  Task 3 partial (builds but CVEs remain)
            0.99  perfect solution

        Anti-cheat: each task has a required-package list. If the agent has
        deleted any of those packages from the manifest, the grader returns
        the floor score regardless of build status. This prevents trivial
        destructive solutions (e.g. removing the broken package instead of
        fixing it).
        """
        obs = self._run_validation_internal()
        ...
        if task_id == 1:
            ...
            return 0.99 if obs.build_status == "SUCCESS" else 0.05
            ...
            if obs.build_status != "SUCCESS":
                ...
                return 0.50  # Builds but still vulnerable
            else:
                return 0.99  # Perfect: builds and no CVEs
